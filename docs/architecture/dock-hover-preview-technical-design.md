...
`DockHoverPreviewProbe` 是一个个人 macOS 菜单栏工具，用公开 API 实现类似 Windows 任务栏窗口预览的最小可用体验：

- 鼠标悬停 Dock 应用图标。
- 等待约 250 ms 后展示该应用在当前可交互环境中可枚举的窗口。
- 预览面板最多展示 8 张横向窗口卡片。
- 每张卡片包含应用图标、窗口标题、静态缩略图或占位图。
- 点击卡片后尝试激活对应窗口，并隐藏面板。

当前状态是 MVP UI `pass with concerns`：普通底部 Dock、单显示器、常规 Space 的核心路径可用；全屏 Space、Stage Manager、Dock 自动隐藏、左右 Dock、多显示器仍需继续验证。
...
- 精确遮挡检测。
- 卡片上的关闭/最小化/全屏按钮。
- 搜索、键盘切换器、Cmd+Tab 替代、设置页、应用过滤、Dock 锁定等扩展功能。
- App Store 分发。
...
- `AppDelegate.swift`：初始化服务、预览会话、菜单栏和 orchestrator。
- `MenuBarController.swift`：安装 `DHP` 菜单，展示权限状态和调试入口。
...
- `ProbeOrchestrator.swift`：负责 250 ms hover 延迟、二次验证、pending work 取消和预览会话启动。
...
- 如果 Dock item 变 stale、候选为空、鼠标离开，都必须取消 pending preview 或隐藏当前面板。
- 鼠标从 Dock 图标移动到 panel 时允许保留面板，但横向移动到相邻 Dock item，尤其是未启动 app，不应保留旧面板。

### 窗口查询

- `WindowQueryService.swift`：用 ScreenCaptureKit 查询当前可见窗口，并用 AX 做 best-effort 匹配。
...
- `PreviewSessionController.swift`：会话状态机，负责权限抑制、窗口查询、缩略图更新、点击激活和鼠标离开轮询。
...
3. `ProbeOrchestrator` 取消旧 pending work，启动 250 ms 延迟。
...
- 每次 ad-hoc 重新签名 app 后，macOS TCC 可能需要重新授权。
